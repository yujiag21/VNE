...
def experiment(time_step=3000, first_try=300,
               ...
               dropout=0, data_path='../data/data_500', load_model_path='', save_model_path='',device='cpu', if_print=False,
               ):
    ...
    return finish_time, period_RC, period_node_UT, 1.0 * success_num / total_num, upline_rc


def test(data_path='../data/data_500', batch_size=10, iteration_num=10, dropout=0, load_model_path='',
         save_model_path='', log_path='', device='cpu', input_type=2, max_request_num=-1,first_try=250):
    '''
    ...
    finish_time, period_RC, period_node_UT, success_ratio, upline_rc = experiment(
        time_step=time_step,
    ...
    if os.path.exists('../logs/result.xlsx'):
        df = pd.read_excel('../logs/result.xlsx')
    else:
        df = pd.DataFrame(
    ...
    df = df.sort_values(by=['max request','batch size','iteration','longterm rc'], ascending=False)
    df.to_excel('../logs/result.xlsx', index=0)
    ...
    df.to_excel('../logs/{}.xlsx'.format(local_time), index=0)
